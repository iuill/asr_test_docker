"""
FastAPI WebSocket Server for Azure Speech-to-Text with Diarization.

This module provides the HTTP and WebSocket endpoints for the
Azure Speech-to-Text service with speaker diarization using ConversationTranscriber.
"""
...
from .transcription_engine import AzureSTTDiarizationEngine, create_engine
...
MODEL_INFO = {
    ...
    "name": "Azure Speech-to-Text (Diarization)",
    "description": "Azure AI Speech SDK with Speaker Diarization",
    "speed": "fast",
}

# Global engine instance
_engine: Optional[AzureSTTDiarizationEngine] = None


def get_engine() -> AzureSTTDiarizationEngine:
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting up Azure STT Diarization service...")
    ...
    logger.info(f"Azure STT Diarization server ready (region: {speech_region})")
...
app = FastAPI(
    title="Azure Speech-to-Text ASR (Diarization)",
    description="Real-time speech recognition with speaker diarization using Azure AI Speech SDK",
    version="0.1.0",
    lifespan=lifespan,
)
...
# Speaker ID to numeric tag mapping
speaker_id_to_tag: dict[str, int] = {}
next_speaker_tag: int = 1


def get_speaker_tag(speaker_id: str) -> int:
    """Convert speaker_id (e.g., 'Guest_1') to numeric tag for webui compatibility."""
    global next_speaker_tag
    if not speaker_id:
        return 0
    if speaker_id not in speaker_id_to_tag:
        speaker_id_to_tag[speaker_id] = next_speaker_tag
        next_speaker_tag += 1
    return speaker_id_to_tag[speaker_id]


def reset_speaker_mapping():
    """Reset speaker ID to tag mapping."""
    global speaker_id_to_tag, next_speaker_tag
    speaker_id_to_tag = {}
    next_speaker_tag = 1
...
async def websocket_asr(websocket: WebSocket):
    """
    WebSocket endpoint for real-time ASR with speaker diarization.
        ...
        async def send_results():
            try:
                while True:
                    ...
                    # Only use speaker_id for final results (intermediate results are unreliable)
                    if result.is_partial:
                        speaker_tag = 0  # Don't show speaker for intermediate results
                    else:
                        speaker_tag = get_speaker_tag(result.speaker_id)
                    response = {
                        "type": "transcription",
                        "text": result.text,
                        ...
                        # Speaker diarization info (numeric tag for webui)
                        "speaker_tag": speaker_tag,
                        "speaker_id": result.speaker_id if not result.is_partial else "",
